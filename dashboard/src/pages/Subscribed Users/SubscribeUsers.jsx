...
const SubscribeUsers = () => {
    ...
    const getAllSubScribers = async () => {
        try {
            setLoading(true);
            const response = await axios.get(`https://skillety-n6r1.onrender.com/all-subscribers?page=${page}`, {
                headers: {
    ...
    //     const selectedMsg = subscribersDetails.find(msg => msg._id === id);
                                ...
                                <div className="row">
                                    <div className="col-12">
                                        <div className="admin-lg-table-section">
                                                ...
                                                <div className='man-app-title-area candidate'>
                                                    <div>
                                                        ...
                                                        <div className="man-app-sub-title">
                                                            Total Subscibers Data :&nbsp;
                                                            <span>{subscribersDetails?.totalPages || 0}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                                {!subscribersDetails ?
                                                    <div className="no-data-created-area">
                                                        <div className='no-data-created'>
                                                            ...
                                                            <div className='no-data-text'>No Subscribers Found Yet..!</div>
                                                        </div>
                                                    </div>
                                                    :
                                                    <div className="table-responsive table-scroll-area">
                                                        <table className="table table-striped table-hover admin-lg-table">
                                                            <tr className='dash-table-row man-app'>
                                                                ...
                                                                <th className='dash-table-head'>Subscribed Date</th>
                                                            ...
                                                            {/* table data */}
                                                            {subscribersDetails?.data.map((user, index) => {
                                                                return (
                                                                    <tr className='dash-table-row client' key={user._id}>
                                                                        <td className='dash-table-data1'>{index + 1}.</td>
                                                                        <td className='dash-table-data1 text-capitalized'>
                                                                            {user.subcribtionDate}
                                                                        </td>
                                            ...
                                            {subscribersDetails && <div className="table-pagination-area pt-3">
                                                <div className="pagination-btn-area">
                                                    {(!(subscribersDetails?.currentPage === subscribersDetails?.totalPages) || subscribersDetails?.currentPage !== 1) && <button className='pag-prev-btn' onClick={() => setpage(page-1)}>
                                                        <i class="bi bi-chevron-left"></i>
                                                    </button>}
                                                    <div className='pag-page'>
                                                        <span className='current-page'>{subscribersDetails?.currentPage}</span>&nbsp;/&nbsp;
                                                        <span className='total-page'>{subscribersDetails?.totalPages}</span>
                                                    </div>
                                                    {!(subscribersDetails?.currentPage === subscribersDetails?.totalPages) && <button className='pag-next-btn' onClick={() => setpage(page+1)}>
